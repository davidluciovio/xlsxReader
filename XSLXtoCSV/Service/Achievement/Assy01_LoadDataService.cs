...
{
    public static class Assy01_LoadDataService
    {
        
        public static void Normalize(string inputFile, string outputFile, int metadataColumnCount = 6)
        {
            ...
            foreach (var row in dataRows)
            {
                ...
                var leader = columns.Length > 1 ? columns[1] : "";
                var partName = metadataColumnCount > 4 ? columns[4] : "";
                var partNumber = metadataColumnCount > 5 ? columns[5] : "";

                // Si la fila está vacía en sus columnas clave, saltar
                if (string.IsNullOrWhiteSpace(supervisor) && string.IsNullOrWhiteSpace(partNumber)) continue;
